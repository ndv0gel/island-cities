...
// Type definition for City object - island is optional since some cities don't have it
interface City {
  name: string;
  population: number;
  island?: string;
 ...
 */
function populationCitiesKai(cityArray: City[]): { [key: string]: number } {
  return cityArray
    .filter(city => city.name.toLowerCase().includes('kai'))
    .reduce((result: { [key: string]: number }, city) => {
 ...
 */
function hasKai(cityArray: City[]): boolean {
...
console.log(
  Object.values(populationCitiesKai(cities)).reduce(
...
console.log(hasKai(cities.filter((city: City) => city.island === 'Lanai')));
...
const citiesWithoutIsland: City[] = [
